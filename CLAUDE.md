...
| `app/services/entitlement_service.py` + `entitlement_gate.py` | Subscription plan limits. The first is the **rules** — pure functions over plan names and counts, no database. The second is **enforcement** — reads the account's state and raises 402. Kept apart so the bands can be re-tuned without touching anything that holds a session. Entitlement resolves to whichever permits more of `owners.granted_plan` (a permanent grant; every account predating billing holds the top tier) and the `subscriptions` row. **Property locking is oldest-first and must stay deterministic** — a random or activity-based choice would let editing one property silently lock another. A locked property is inaccessible, not read-only (PLATFORM.md §19): every read filters through `EntitlementGate.hidden_property_ids`, and single-record paths call `require_property_unlocked`. Two deliberate holes: `export_service` reads repositories directly so the export keeps everything, and `get_notification_engine` builds its `RenterService` **without** the gate — the engine's candidates feed `_dismiss_resolved`, so hiding locked renters there would dismiss their alerts for good |
